...
class AudioEditorApp {
  ...
  bindCutPanelEvents() {
    ...
    // 均衡器控制
    this.bindCutEqualizerEvents();
  }

  /**
   * 绑定剪切面板均衡器事件
   */
  bindCutEqualizerEvents() {
    const eqLow = document.getElementById("cut-eq-low");
    const eqMid = document.getElementById("cut-eq-mid");
    const eqHigh = document.getElementById("cut-eq-high");
    const eqReset = document.getElementById("cut-eq-reset");

    const updateEqValue = (slider, valueId) => {
    ...
    eqLow.addEventListener("input", () => {
      window.audioEngine.setLowGain(parseFloat(eqLow.value));
      updateEqValue(eqLow, "cut-eq-low-value");
    });

    eqMid.addEventListener("input", () => {
      window.audioEngine.setMidGain(parseFloat(eqMid.value));
      updateEqValue(eqMid, "cut-eq-mid-value");
    });

    eqHigh.addEventListener("input", () => {
      window.audioEngine.setHighGain(parseFloat(eqHigh.value));
      updateEqValue(eqHigh, "cut-eq-high-value");
    });

    eqReset.addEventListener("click", () => {
      window.audioEngine.resetEqualizer();
      eqLow.value = 0;
      eqMid.value = 0;
      eqHigh.value = 0;
      updateEqValue(eqLow, "cut-eq-low-value");
      updateEqValue(eqMid, "cut-eq-mid-value");
      updateEqValue(eqHigh, "cut-eq-high-value");
      window.uiManager.showToast("均衡器已重置", "info");
    });
  }
  ...
  bindMergePanelEvents() {
    ...
    // 预览和导出
    document
      .getElementById("merge-preview-btn")
      .addEventListener("click", () => {
        this.previewMerge();
      });

    document
      .getElementById("merge-export-btn")
      .addEventListener("click", () => {
        this.exportMergeAudio();
      });

    // 均衡器控制
    this.bindMergeEqualizerEvents();
  }

  /**
   * 绑定合并面板均衡器事件
   */
  bindMergeEqualizerEvents() {
    const eqLow = document.getElementById("merge-eq-low");
    const eqMid = document.getElementById("merge-eq-mid");
    const eqHigh = document.getElementById("merge-eq-high");
    const eqReset = document.getElementById("merge-eq-reset");
    ...
    eqLow.addEventListener("input", () => {
      window.audioEngine.setLowGain(parseFloat(eqLow.value));
      updateEqValue(eqLow, "merge-eq-low-value");
    });

    eqMid.addEventListener("input", () => {
      window.audioEngine.setMidGain(parseFloat(eqMid.value));
      updateEqValue(eqMid, "merge-eq-mid-value");
    });

    eqHigh.addEventListener("input", () => {
      window.audioEngine.setHighGain(parseFloat(eqHigh.value));
      updateEqValue(eqHigh, "merge-eq-high-value");
    });
  ...
  async loadCutAudio(file) {
    try {
      ...
      document.getElementById("cut-end-time").value =
  ...
  drawCutWaveform() {
    const canvas = document.getElementById("cut-waveform");
    const startTime =
      parseFloat(document.getElementById("cut-start-time").value) || 0;
    const endTime =
      parseFloat(document.getElementById("cut-end-time").value) ||
      this.cutState.audioBuffer.duration;
    const duration = this.cutState.audioBuffer.duration;
  ...
  updateCutSelection() {
    ...
    window.uiManager.updateSelection(
      selection,
      startTime / duration,
      endTime / duration,
    );
  ...
  async playCutAudio() {
    ...
    const equalizer = window.audioEngine.createEqualizer();
    this.cutState.sourceNode.connect(equalizer.input);
    equalizer.output.connect(ctx.destination);
    ...
    window.uiManager.updatePlayButton(
      document.getElementById("cut-play-btn"),
      true,
    );
  ...
  stopCutPlayback() {
    ...
    window.uiManager.updateTimeDisplay(
      document.getElementById("cut-current-time"),
      0,
    );
  ...
  updateCutPlayhead() {
    ...
    window.uiManager.updatePlayhead(
      document.getElementById("cut-playhead"),
      percent,
    );
    window.uiManager.updateTimeDisplay(
      document.getElementById("cut-current-time"),
    ...
    this.cutState.animationFrame = requestAnimationFrame(() =>
      this.updateCutPlayhead(),
    );
  }
  ...
  async previewCutSelection() {
    ...
    try {
      ...
      window.uiManager.showToast(
        `正在播放选区 ${startTime.toFixed(2)}s - ${endTime.toFixed(2)}s`,
        "info",
      );
  ...
  async exportCutAudio() {
    ...
    try {
      ...
      // 剪切音频
      const cutBuffer = window.audioEngine.cutAudio(
        this.cutState.audioBuffer,
        startTime,
        endTime,
      );
      ...
      await window.audioExporter.export(
        cutBuffer,
        format,
        filename,
        (progress) => {
          window.uiManager.showLoading(
            `正在编码... ${Math.round(progress * 100)}%`,
          );
        },
      );
  ...
  async loadMergeAudio(trackNum, file) {
    try {
      ...
      // 隐藏上传区域，显示信息
      document.getElementById(`merge-upload-${trackNum}`).style.display =
        "none";
      ...
      document.getElementById(`merge-duration-${trackNum}`).textContent =
        info.durationFormatted;
  ...
  removeMergeAudio(trackNum) {
    ...
    this.mergeState.mergedBuffer = null;
    document.getElementById("merge-preview").style.display = "none";
    document.getElementById("merge-equalizer").style.display = "none";
  }
  ...
  async previewMerge() {
    ...
    try {
      ...
      document.getElementById("merge-equalizer").style.display = "block";

      // 更新时长显示
      const info = window.audioEngine.getAudioInfo(
        this.mergeState.mergedBuffer,
      );
      document.getElementById("merge-duration").textContent =
        info.durationFormatted;
  ...
  async playMergeAudio() {
    ...
    const equalizer = window.audioEngine.createEqualizer();
    this.mergeState.sourceNode.connect(equalizer.input);
    equalizer.output.connect(ctx.destination);
    ...
    window.uiManager.updatePlayButton(
      document.getElementById("merge-play-btn"),
      true,
    );
  ...
  stopMergePlayback() {
    ...
    window.uiManager.updateTimeDisplay(
      document.getElementById("merge-current-time"),
      0,
    );
  ...
  updateMergePlayhead() {
    ...
    this.mergeState.animationFrame = requestAnimationFrame(() =>
      this.updateMergePlayhead(),
    );
  }
